...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['delete_id'])) {
    ...
    $sql = "DELETE FROM contact_us WHERE Contact_ID = ?";
    $stmt = $con->prepare($sql);
    $stmt->bind_param("i", $delete_id);

    if ($stmt->execute()) {
        echo "<script>alert('Feedback deleted successfully'); window.location.href='Admin_ViewFeedback.php';</script>";
    } else {
        echo "<script>alert('Error deleting feedback'); window.location.href='Admin_ViewFeedback.php';</script>";
    }
            ...
            <tbody id="feedback-table">
                ...
                if ($result->num_rows > 0) {
                    ...
                    while($row = $result->fetch_assoc()) {
                        ...
                        echo "<td><button class='delete-btn' data-id='" . $row["Contact_ID"] . "'><i class='fa fa-trash'></i></button></td>";
                        echo "</tr>";
                        $counter++;
                    }
                } else {
