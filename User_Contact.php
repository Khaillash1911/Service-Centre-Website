...
if (isset($_POST['sendfeedbackBtn'])) {
  ...
  if(empty($name) || empty($email) || empty($message) || empty($mood)) {
    echo "<script>alert('Please fill in all the fields');window.location.href='User_Contact.php';</script>";
    exit;
  }

  // Validate email format
  if(!filter_var($email, FILTER_VALIDATE_EMAIL)) {
    echo "<script>alert('Invalid email format');window.location.href='User_Contact.php';</script>";
    exit;
  }
...
<body>
  <nav>
    <?php include 'navbar/User_Navbar.html';?>
  </nav>
  <div>
